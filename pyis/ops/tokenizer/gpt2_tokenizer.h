...
#else
                    auto search_it =
                        std::search(it, str.first.end(), std::boyer_moore_searcher(st.str.begin(), st.str.end()));
#endif
...
class VocabData {
  public:
    ...
    void Load(std::istream& vocab_stream, std::istream& merges_stream, const char* unk_token,
              const char* special_tokens) {
        rapidjson::Document tok_json;
        tok_json.ParseStream(vocab_stream);
        vocab_map_.clear();
    ...
    auto SplitBySpeicalTokens(const std::string& input) const { return special_tokens_.SplitBySpeicalTokens(input); }
